...
public class SmartTaxi extends Actor implements Taxi {
        ...
        if (!companyIsReached) {
            ...
            if (this.canMove()) {
                ...
                if (!isOccupied())
                    ...
                    this.move(convertToLocation(requestSet.get(0).originCompanyId));

                else
                    this.move(convertToLocation(requestSet.get(0).destinationCompanyId));
    ...
    int processRequest(String origin, String destination) {
        ...
        int[] dirs = {Location.AHEAD, Location.RIGHT, Location.LEFT, Location.HALF_LEFT, Location.HALF_RIGHT, Location.HALF_CIRCLE
                , Location.NORTHWEST, Location.SOUTHEAST, Location.SOUTHWEST, Location.WEST};
    ...
    public void move(Location target) {
        ...
        if (gr != null) {
            ...
            Location loc = this.getLocation();
            int goal = loc.getDirectionToward(target);
            Location next = loc.getAdjacentLocation(goal);
            if (gr.isValid(next)) {

                Actor neighbor = (Actor) gr.get(next);
                if (!(neighbor instanceof SmartTaxi))
    ...
    public boolean canMove() {
        ...
        if (gr == null) {
            ...
            if (!gr.isValid(next)) {
                ...
                Actor neighbor = (Actor) gr.get(next);
                if (neighbor == null || !(neighbor instanceof SmartTaxi) && !(neighbor instanceof Company)) {
